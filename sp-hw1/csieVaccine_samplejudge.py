...
class Tasks:
	...
	def task1(self):
		try:
			...
			logging.info('\033[91m'+'task1: make command failed.'+'\033[0m')
			logging.info(f'Here is the original error msg: {err}')
		else:
			try:
				#logging.info()
				...
				logging.info('\033[91m'+'task1: start read_server/write_server failed.'+'\033[0m')
				raise
			else:
				try:
					#logging.info()
					...
					write_proc.kill()
					write_proc.wait()		
					logging.info('\33[32m'+"===task1 succeed!==="+"\33[0m")	
	...
	def task2(self):
		...
		try:
			...
			try:
				...
				logging.info("\33[3m"+"(Client side input) " +"\33[0m"+"902001")
				...
				logging.info("\33[32m"+"===task2-1 succeed!==="+"\33[0m")
			finally:
				read_proc.kill()
				read_proc.wait()
	...
	def task2_2(self):
		...
		try:
			...
			try:
				...
				logging.info("\33[3m"+"(Client side input) " +"\33[0m"+"999999")
				...
				logging.info("\33[32m"+"===task2-2 succeed!==="+"\33[0m")
			finally:
				read_proc.kill()
				read_proc.wait()
	...
	def task3(self):
		...
		try:
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			logging.info("\33[3m"+"(Server output)"+gets)
			...
			logging.info("\33[3m"+"(Client side input) " +"\33[0m"+"2 1 3")
			...
			logging.info("\33[32m"+"===task3-1 succeed!==="+"\33[0m")
		finally:
			write_proc.kill()
			write_proc.wait()
	...
	def task6(self):
		...
		try:
			...
			logging.info("\33[3m"+"(Server2 output)"+gets)
			assert (gets == self.str1), '\033[91m'+"task6-1 Failed. Should be\n"+'\033[0m'+self.str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'
			
			logging.info("\33[3m"+"(Client1 side input) " +"\33[0m"+"902001")
			telnet_client.write(b"902001\n")
			gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
			logging.info("\33[3m"+"(Server output)"+gets)
			assert (gets == task6_str1), '\033[91m'+"task6-1 Failed. Should be\n"+'\033[0m'+task6_str1+'\033[91m'+"\nbut have\n"+'\033[0m'+gets+'\033[91m'+"\ninstead."+'\033[0m'

			logging.info("\33[3m"+"(Client2 side input) " +"\33[0m"+"902001")
			...
			logging.info("\33[3m"+"(Client1 side input) " +"\33[0m"+"3 2 1")
			...
			logging.info("\33[32m"+"===task6-1 succeed!==="+"\33[0m")
		finally:
			write_proc.kill()
			write_proc.wait()	
			write_proc2.kill()
...
def main():
	...
	logging.info("=== Making copy of registerRecord...===")
	...
	try:
		...
		logging.info("=== Restoring copy of registerRecord...===")
		cmd = subprocess.Popen(["cp", "registerRecord_copy", "registerRecord"])
		cmd = subprocess.Popen(["rm", "registerRecord_copy"])
		cmd = subprocess.Popen(["rm", "read_server"])
		cmd = subprocess.Popen(["rm", "write_server"])

		logging.info('\33[1m'+"Note that, this is a sample judge and it does not contain all testcases/sub-testcases,\nso passing the judge does not guarantee that you get full points of the task(s)."+"\33[0m")
